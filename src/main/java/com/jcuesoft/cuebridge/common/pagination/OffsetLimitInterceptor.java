...
import com.jcuesoft.cuebridge.common.pagination.support.Dialect;
import com.jcuesoft.cuebridge.common.pagination.support.PropertiesHelper;
import com.jcuesoft.cuebridge.common.pagination.support.SQLHelp;
...
public class OffsetLimitInterceptor implements Interceptor {
    ...
    public Object intercept(final Invocation invocation) throws Throwable {
        String cdCompnay = CommonSessionUtil.getCdCompany();
        logger.debug("Call OffsetLimitInterceptor intercept...");
        //logger.debug("!!!!!!!!!!!!!!!!compnay:"+cdCompnay);
        ...
        queryArgs[MAPPED_STATEMENT_INDEX] = copyFromNewSql(ms, boundSql, dialect.getPageSQL(), dialect.getParameterMappings(),
        ...
        /*
         * Callable<List<?>> listFuture = new Callable<List<?>>() { public
         * Object call() throws Exception { Integer count; Cache cache =
         * ms.getCache(); if (cache != null && ms.isUseCache()) { CacheKey
         * cacheKey = executor.createCacheKey( ms, parameter, new PageBounds(),
         ...
         * cache.putObject(cacheKey, count); } } else { count =
         * SQLHelp.getCount(ms, parameter, boundSql, dialect); } return new
         * Paginator(pageBounds.getPage(), pageBounds.getLimit(), count); } };
         */
//
//        Future<List<?>> listFuture = call(new Callable<List<?>>() {
//            public List<?> call() throws Exception {
//                List<?> list;
//                list = (List<?>) SQLHelp.getList(ms, parameter, boundSql);
//                return list;
//            }
//        }, async);

        Future<List<?>> listFuture = call(new Callable<List<?>>() {
            public List<?> call() throws InvocationTargetException, IllegalAccessException {
                logger.debug("@@@@@@@@@@listFuture exception");
                List<?> list;
            ...
            Callable<Paginator> countTask = new Callable() {
                public Object call() throws Exception {
                    ...
                    if (cache != null && ms.isUseCache()) {
                        ...
                        if (count == null) {
                            count = SQLHelp.getCount(ms, parameter, boundSql, dialect, cdCompnay);
...
//                logger.equals(" OffsetLimitInterceptor listFuture.get():" + listFuture.get());
//                logger.equals(" OffsetLimitInterceptor countFutrue.get():" + countFutrue.get());
//
//            }
            return new PageList(listFuture.get(), countFutrue.get());
        }
        
        return new PageList(listFuture.get());
...
//        return listFuture.get();
    ...
    private MappedStatement copyFromNewSql(MappedStatement ms, BoundSql boundSql, String sql, List<ParameterMapping> parameterMappings,
            Object parameter) {
        BoundSql newBoundSql = copyFromBoundSql(ms, boundSql, sql, parameterMappings, parameter);
        return copyFromMappedStatement(ms, new BoundSqlSqlSource(newBoundSql));
    }

    private BoundSql copyFromBoundSql(MappedStatement ms, BoundSql boundSql, String sql, List<ParameterMapping> parameterMappings,
            Object parameter) {
